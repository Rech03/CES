...
def get_quiz_recommendations(request):
    """
    Get personalized quiz recommendations for student based on performance and progress
    """
    student = request.user
    ...
    try:
        ...
        all_progress = StudentAdaptiveProgress.objects.filter(
            ...
            'adaptive_quiz__lecture_slide__topic__course'
        )
        ...
        # 1. Incomplete easy quizzes (highest priority)
        incomplete_easy = AdaptiveQuiz.objects.filter(
            lecture_slide__topic__course_id__in=enrolled_course_ids,
            difficulty='easy',
            status='published',
            is_active=True
        ).exclude(
            id__in=all_progress.filter(completed=True).values_list('adaptive_quiz_id', flat=True)
        ).select_related('lecture_slide__topic__course')[:3]
        
        for quiz in incomplete_easy:
            recommendations.append({
                'quiz_id': quiz.id,
                'slide_title': quiz.lecture_slide.title,
                'course_code': quiz.lecture_slide.topic.course.code,
                'difficulty': quiz.difficulty,
                'recommendation_type': 'start_easy',
                'reason': 'Start with this easy quiz to build confidence',
                'priority': 1
            })
        
        # 2. Available medium quizzes (after completing easy)
        completed_easy_slides = all_progress.filter(
            adaptive_quiz__difficulty='easy',
            completed=True
        ).values_list('adaptive_quiz__lecture_slide_id', flat=True)
        
        available_medium = AdaptiveQuiz.objects.filter(
            lecture_slide__id__in=completed_easy_slides,
            difficulty='medium',
            status='published',
            is_active=True
        ).exclude(
        ...
        for quiz in available_medium:
            recommendations.append({
                'quiz_id': quiz.id,
                'slide_title': quiz.lecture_slide.title,
                'course_code': quiz.lecture_slide.topic.course.code,
                'difficulty': quiz.difficulty,
                'recommendation_type': 'progress_medium',
                'reason': 'You completed the easy level - try medium difficulty',
                'priority': 2
            })
        
        # 3. Retake low-scoring quizzes
        low_scoring = all_progress.filter(
            completed=True,
            best_score__lt=70
        ).order_by('best_score')[:2]
        
        for progress in low_scoring:
            recommendations.append({
                'quiz_id': progress.adaptive_quiz.id,
                'slide_title': progress.adaptive_quiz.lecture_slide.title,
                'course_code': progress.adaptive_quiz.lecture_slide.topic.course.code,
                'difficulty': progress.adaptive_quiz.difficulty,
                'recommendation_type': 'improve_score',
                'reason': f'Retake to improve your score of {progress.best_score}%',
                'priority': 3,
                'current_score': progress.best_score
            })
