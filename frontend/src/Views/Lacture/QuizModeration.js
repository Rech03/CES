...
const QuizModeration = () => {
  ...
  const loadQuizForModeration = async (coursesData = courses) => {
    ...
    try {
      ...
      if (!finalQuizData) throw new Error('No quiz data available');

      let courseInfo = { code: 'Unknown Course', name: 'Unknown Course' };
      let topicInfo = { name: 'Unknown Topic' };

      if (finalQuizData.topic) {
        topicInfo = { name: finalQuizData.topic.name || finalQuizData.topic_name || 'Unknown Topic' };
        ...
        topicInfo = { name: finalQuizData.subject.name || 'Unknown Topic' };
      ...
      if ((courseInfo.code === 'UNKNOWN' || topicInfo.name === 'Unknown Topic') && finalQuizData.title) {
        ...
        if (courseCodeMatch && courseInfo.code === 'UNKNOWN') {
          ...
          courseInfo = { code: detectedCode, name: `Course ${detectedCode}` };
          if (coursesData && coursesData.length > 0) {
            const matchingCourse = coursesData.find(c => c.code && c.code.toUpperCase() === detectedCode.toUpperCase());
            if (matchingCourse) {
              courseInfo = { code: matchingCourse.code, name: matchingCourse.name };
            }
          }
        }
      ...
      const errorMsg = err.response?.data?.detail || err.response?.data?.message || 'Failed to load quiz for moderation';
      setError(errorMsg);
    } finally {
      setLoading(false);
    }
  ...
  const addChoice = (questionIndex) => {
    ...
    setQuestions(prev => {
      const updated = [...prev];
      const newChoice = { choice_text: '', is_correct: false, order: updated[questionIndex].choices.length };
      updated[questionIndex] = { ...updated[questionIndex], choices: [...updated[questionIndex].choices, newChoice] };
      return updated;
    });
    setHasChanges(true);
  };
  ...
  const toBackendPayload = (title, uiQuestions) => {
    const mapped = uiQuestions.map((q) => {
      const kept = (q.choices || [])
        .filter((c) => (c.choice_text || '').trim().length > 0)
        .map((c, i) => ({ ...c, _k: LETTERS[i] }));
      const correct = kept.find((c) => c.is_correct) || kept[0];
      const options = kept.reduce((acc, c) => {
        acc[c._k] = c.choice_text.trim();
        return acc;
      }, {});
      return {
        ...
        options,
        correct_answer: correct ? correct._k : 'A',
        explanation: q.explanation ?? ''
      };
    });
    return { title: (title || '').trim(), questions: mapped };
  };
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      const payload = toBackendPayload(quiz.title, questions);
      await updateQuizQuestions(operationalId, payload);
      setSuccess('Changes saved successfully!');
      setHasChanges(false);
    } catch (err) {
      const errorMsg = err.response?.data?.detail || err.response?.data?.message || 'Failed to save changes';
      setError(errorMsg);
    } finally {
      setSaving(false);
    }
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      setSuccess(publishResponse?.message || 'Quiz published successfully! Students can now access it. Redirecting to dashboard...');
      setTimeout(() => navigate('/LecturerDashboard'), 3000);
    } catch (err) {
      const errorMsg = err.response?.data?.detail || err.response?.data?.message || 'Failed to publish quiz';
      setError(errorMsg);
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="quiz-moderation-container">
      <div className="moderation-content">
        <div className="moderation-header">
          <div className="header-top">
            ...
            <div className="quiz-meta">
              <span className="course-code">{quiz?.topic?.course?.code}</span>
              <span className="topic-name">{quiz?.topic?.name}</span>
              <span className="difficulty-badge" style={{ backgroundColor: getDifficultyColor(quiz?.difficulty) }}>
                {quiz?.difficulty || 'Medium'}
              </span>
              {isPublishedQuiz && <span className="published-badge">Published</span>}
            </div>
          </div>
          <div className="header-actions">
            <button onClick={() => navigate('/LecturerDashboard')} className="btn-secondary" disabled={saving}>
              {isPublishedQuiz ? 'Back to Dashboard' : 'Cancel'}
            </button>
            {!isPublishedQuiz && (
              <button onClick={handleSaveChanges} className="btn-primary" disabled={saving || !hasChanges}>
                {saving ? 'Saving...' : 'Save Changes'}
              </button>
            )}
            {!isPublishedQuiz && (
              <button onClick={handlePublish} className="btn-success" disabled={saving}>
        ...
        {isPublishedQuiz && <div className="alert alert-info">This quiz has been published and is available to students. Editing is disabled to maintain consistency.</div>}
        ...
        <div className="quiz-details-card">
          ...
          <div className="quiz-stats-grid">
            ...
            <div className="stat-item">
              ...
              <span className="stat-value" style={{ color: quiz?.status === 'published' ? '#27AE60' : '#F39C12' }}>
                {quiz?.status || 'Unknown'}
              </span>
            </div>
            <div className="stat-item">
        ...
        <div className="questions-container">
          ...
          {questions.length === 0 ? (
            <div className="empty-questions">
              ...
              <p>{!isPublishedQuiz ? 'Add your first question to get started with this quiz.' : 'This quiz has no questions.'}</p>
              {!isPublishedQuiz && <button onClick={addQuestion} className="btn-primary">Add First Question</button>}
            </div>
          ) : (
                ...
                <div key={qIndex} className="question-card">
                  <div className="question-header">
                    ...
                    <div className="question-controls">
                      <select
                        ...
                        onChange={(e) => !isPublishedQuiz && handleQuestionChange(qIndex, 'difficulty', e.target.value)}
                        className="difficulty-select"
                        disabled={isPublishedQuiz}
                      >
                      ...
                      <div className="points-group">
                        <input
                          ...
                          onChange={(e) => !isPublishedQuiz && handleQuestionChange(qIndex, 'points', parseInt(e.target.value) || 1)}
                      ...
                      {!isPublishedQuiz && (
                        <button onClick={() => removeQuestion(qIndex)} className="btn-remove-question" title="Remove question">×</button>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="question-content">
                    ...
                    <textarea
                      ...
                      onChange={(e) => !isPublishedQuiz && handleQuestionChange(qIndex, 'question_text', e.target.value)}
                      ...
                      {question.choices?.map((choice, cIndex) => (
                        <div key={cIndex} className="choice-item">
                          <input
                            ...
                            onChange={() => !isPublishedQuiz && handleCorrectAnswerChange(qIndex, cIndex)}
                          ...
                          <input
                            ...
                            onChange={(e) => !isPublishedQuiz && handleChoiceChange(qIndex, cIndex, 'choice_text', e.target.value)}
                            placeholder={`Choice ${String.fromCharCode(65 + cIndex)}`}
                            className="choice-input"
                            disabled={isPublishedQuiz}
                          />
                          {!isPublishedQuiz && question.choices.length > 2 && (
                            <button onClick={() => removeChoice(qIndex, cIndex)} className="btn-remove-choice" title="Remove choice">×</button>
                          )}
                        </div>
                      ))}
                    </div>
                    {!isPublishedQuiz && question.choices?.length < 6 && (
                      <button onClick={() => addChoice(qIndex)} className="btn-add-choice">+ Add Choice</button>
                    )}
                  </div>
                </div>
              ))}
        ...
        <div className="moderation-footer">
          <div className="footer-content">
            ...
            {hasChanges && !isPublishedQuiz && <p className="unsaved-changes">You have unsaved changes</p>}
            {isPublishedQuiz && <p className="read-only-notice">This published quiz is read-only</p>}
          </div>
        </div>
      </div>
    </div>
